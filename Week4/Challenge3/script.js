...
function show(button_element) {
    ...
    */
    ...
    const funFact = facts[name]
    document.getElementById('ta-fun-fact').innerHTML = funFact
    document.getElementById('ta-photo').src = './photos/' + name + '.jpg';
    for (item of document.getElementById('left-child').children) {
        if(item.id !== ""){
            if(item.id == name){
                document.getElementById(item.id).style.backgroundColor = 'lightblue'
                document.getElementById(item.id).style.color="black"
            }else{
                document.getElementById(item.id).style.backgroundColor = 'darkblue'
